...
var liveTrackingWatchId = null;
var liveTrackingSeconds = 0;
var liveTrackingInterval = null;

(function initLiveLocationPage() {
  var trackingCard = document.getElementById('tracking-card');
  if (!trackingCard) return; // Not on live location page
...
function startLiveTracking() {
  if (!navigator.geolocation) {
    showToast('⚠️ GPS not supported in this browser.', 'error');
    return;
  }
  ...
  liveTrackingWatchId = navigator.geolocation.watchPosition(
    function (pos) {
      ...
      var latEl = document.getElementById('live-lat');
      var lngEl = document.getElementById('live-lng');
      var accEl = document.getElementById('live-accuracy');
    ...
    function (err) {
      ...
      if (mapOverlay) {
        mapOverlay.innerHTML = '<div class="map-loading"><span>📍 GPS unavailable — ' + err.message + '</span></div>';
      }
    },
  ...
  // Start timer
  liveTrackingSeconds = 0;
  var timerEl = document.getElementById('tracking-timer');
  liveTrackingInterval = setInterval(function () {
    liveTrackingSeconds++;
    if (timerEl) timerEl.textContent = formatTime(liveTrackingSeconds);
  }, 1000);

  // Update button
  var btn = document.getElementById('tracking-toggle-btn');
  if (btn) {
...
function stopLiveTracking() {
  ...
  // Update UI
  var statusLabel = document.getElementById('tracking-label');
  if (statusLabel) statusLabel.textContent = 'TRACKING STOPPED';

  var dot = document.querySelector('.tracking-dot');
  if (dot) dot.style.background = 'var(--gray-500)';
...
function toggleLiveTracking() {
  if (liveTrackingActive) {
    ...
    var statusLabel = document.getElementById('tracking-label');
    if (statusLabel) statusLabel.textContent = 'TRACKING ACTIVE';
    var dot = document.querySelector('.tracking-dot');
    if (dot) dot.style.background = '';
  }
}

// Share functions for live location page
function buildLocationMessage() {
  var lat = currentLat || '—';
  var lng = currentLng || '—';
  var mapsLink = 'https://www.google.com/maps?q=' + lat + ',' + lng;
  return '📍 SHEild AI — Live Location\n' +
    'Latitude: ' + lat + '\n' +
    'Longitude: ' + lng + '\n' +
    '🗺️ Google Maps: ' + mapsLink + '\n' +
...
function shareLiveToWhatsApp() {
  if (!currentLat) { showToast('⏳ Waiting for GPS...', 'info'); return; }
  var msg = buildLocationMessage();
  // Simply add message to in-app chat instead of external WhatsApp
  if (typeof addInAppMessage === 'function') {
    addInAppMessage(msg, 'me', 'location');
    showToast('💬 Location shared via in-app chat!', 'success');
  } else {
    // Fallback if chat not loaded
    var history = JSON.parse(localStorage.getItem('sheild_chat_history') || '[]');
    history.push({
      text: msg,
      sender: 'me',
      type: 'location',
    ...
    localStorage.setItem('sheild_chat_history', JSON.stringify(history));
    showToast('💬 Location shared via in-app chat!', 'success');
  }
}

function shareLiveToSMS() {
  if (!currentLat) { showToast('⏳ Waiting for GPS...', 'info'); return; }
  var msg = buildLocationMessage();
  if (typeof addInAppMessage === 'function') {
    addInAppMessage(msg, 'me', 'location');
...
function copyLocationToClipboard() {
  ...
  navigator.clipboard.writeText(msg).then(function () {
    ...
    showToast('📍 LAT: ' + currentLat + ', LNG: ' + currentLng, 'info');
  });
}

function shareLocationNative() {
  if (!currentLat) { showToast('⏳ Waiting for GPS...', 'info'); return; }
  var mapsLink = 'https://www.google.com/maps?q=' + currentLat + ',' + currentLng;
  if (navigator.share) {
    navigator.share({
      title: '📍 My Live Location — SHEild AI',
      text: buildLocationMessage(),
      url: mapsLink
    }).then(function () {
...
// =============================================
// EMERGENCY PAGE SHARE BUTTONS
// =============================================
function emergencyShareWhatsApp() {
  if (!currentLat) {
    getRealLocation(function (lat, lng) {
      doEmergencyShareWA(lat, lng);
    });
  } else {
    doEmergencyShareWA(currentLat, currentLng);
  }
}

function doEmergencyShareWA(lat, lng) {
  var msg = buildEmergencyMessage(lat, lng);
  if (typeof addInAppMessage === 'function') {
    addInAppMessage(msg, 'me', 'alert');
  } else {
    var history = JSON.parse(localStorage.getItem('sheild_chat_history') || '[]');
    history.push({
      text: msg,
  ...
  showToast('💬 Emergency shared via in-app chat!', 'success');
}

function emergencyShareSMS() {
  if (!currentLat) {
    getRealLocation(function (lat, lng) {
      doEmergencyShareSMS(lat, lng);
    });
  } else {
    doEmergencyShareSMS(currentLat, currentLng);
  }
}

function doEmergencyShareSMS(lat, lng) {
  var msg = buildEmergencyMessage(lat, lng);
  if (typeof addInAppMessage === 'function') {
    addInAppMessage(msg, 'me', 'alert');
  } else {
...
// Emergency page — share audio
var emergencyAudioStream = null;
var emergencyAudioRecorder = null;
var emergencyAudioChunks = [];

function shareEmergencyAudio() {
  navigator.mediaDevices.getUserMedia({ audio: true })
    .then(function (stream) {
      emergencyAudioStream = stream;
      emergencyAudioRecorder = new MediaRecorder(stream);
      emergencyAudioChunks = [];

      emergencyAudioRecorder.ondataavailable = function (e) {
        if (e.data && e.data.size > 0) emergencyAudioChunks.push(e.data);
      };

      emergencyAudioRecorder.onstop = function () {
        var blob = new Blob(emergencyAudioChunks, { type: 'audio/webm' });
        // Download the audio
        downloadFile(blob, 'SHEild_EmergencyAudio_' + Date.now() + '.webm');
        
        // Share via in-app chat
        var lat = currentLat || '—';
        var lng = currentLng || '—';
        var msg = '🎤 SHEild AI — Audio Evidence Recorded!\n\n' +
          '📍 Location: https://www.google.com/maps?q=' + lat + ',' + lng + '\n' +
          '⏰ Time: ' + new Date().toLocaleString() + '\n' +
          'Audio file has been saved. Please check on me!';
        
        if (typeof addInAppMessage === 'function') {
          addInAppMessage(msg, 'me', 'audio');
        } else {
          var history = JSON.parse(localStorage.getItem('sheild_chat_history') || '[]');
          history.push({
            text: msg,
        ...
        showToast('🎤 Audio saved & shared in chat!', 'success');
      };

      emergencyAudioRecorder.start(1000);
      showToast('🎤 Recording audio — tap again to stop & share', 'error');

      // Auto-stop after 30 seconds
      setTimeout(function () {
        if (emergencyAudioRecorder && emergencyAudioRecorder.state !== 'inactive') {
          emergencyAudioRecorder.stop();
          if (emergencyAudioStream) {
            emergencyAudioStream.getTracks().forEach(function (t) { t.stop(); });
          }
        }
      }, 30000);
    })
    .catch(function () {
      showToast('⚠️ Mic access denied.', 'error');
    });
}

// =============================================
// IN-APP CHAT SYSTEM (No external apps)
// =============================================
var activeChatContact = 'all';
var chatAudioStream = null;
var chatAudioRecorder = null;
var chatAudioChunks = [];
var chatAudioTimerInterval = null;
var chatAudioSeconds = 0;

(function initChatPage() {
  var chatWindow = document.getElementById('chat-window');
  if (!chatWindow) return; // Not on chat page

  // Build contact tabs
  var tabsContainer = document.getElementById('contact-tabs');
  if (tabsContainer) {
    var contacts = getContacts();
    contacts.forEach(function (c, i) {
      var tab = document.createElement('button');
      tab.className = 'chat-tab';
      tab.setAttribute('data-contact', i);
      tab.textContent = '👤 ' + c.name;
      tab.onclick = function () { switchChatContact(String(i), tab); };
...
function switchChatContact(contactId, tabEl) {
...
function getChatHistory() {
  return JSON.parse(localStorage.getItem('sheild_chat_history') || '[]');
}
...
function addInAppMessage(text, sender, type) {
  // sender: 'me' or contact name
  // type: 'text', 'location', 'audio', 'alert'
  var history = getChatHistory();
  var contacts = getContacts();
  var contactName = 'All Contacts';
  if (activeChatContact !== 'all') {
    var idx = parseInt(activeChatContact, 10);
    if (contacts[idx]) contactName = contacts[idx].name;
  }
  ...
  history.push({
    ...
    sender: sender,
    type: type || 'text',
    contactTarget: activeChatContact,
...
function renderInAppChat() {
  ...
  // Filter by active contact
  var filtered = history.filter(function (msg) {
    if (activeChatContact === 'all') return true;
    return msg.contactTarget === activeChatContact || msg.contactTarget === 'all';
  });
  ...
  filtered.forEach(function (msg) {
    ...
    var typeIcon = '';
    if (msg.type === 'location') typeIcon = '📍 ';
    else if (msg.type === 'audio') typeIcon = '🎤 ';
    else if (msg.type === 'alert') typeIcon = '🚨 ';

    bubble.innerHTML =
      '<div class="chat-bubble-text">' + typeIcon + escapeHtml(msg.text) + '</div>' +
      '<div class="chat-bubble-meta">' +
      '<span>' + (isSent ? '→ ' + escapeHtml(msg.contactName) : escapeHtml(msg.sender)) + '</span>' +
      '<span>' + msg.time + '</span>' +
      '</div>';
...
function sendLocationMessage() {
  ...
  getRealLocation(function (lat, lng) {
    var mapsLink = 'https://www.google.com/maps?q=' + lat + ',' + lng;
    var locText = 'My Location:\nLat: ' + lat + ', Lng: ' + lng + '\nMap: ' + mapsLink;
    addInAppMessage(locText, 'me', 'location');
    showToast('📍 Location sent!', 'success');
    simulateAutoReply('location_shared');
  });
}

// === Simulated auto-reply from contacts ===
function simulateAutoReply(triggerText) {
  var contacts = getContacts();
  if (contacts.length === 0) return;

  var contact = contacts[0]; // First contact replies
  if (activeChatContact !== 'all') {
    var idx = parseInt(activeChatContact, 10);
    if (contacts[idx]) contact = contacts[idx];
  }

  var replies = {
    'help': '🏃 I\'m on my way! Stay safe and keep this app open.',
    'safe': '😊 So glad to hear that! Take care of yourself.',
    'call': '📞 Calling you now! Pick up!',
    'unsafe': '⚠️ I\'m alerting others too. Stay in a public area if possible.',
    'location': '📍 Got your location! I can see where you are.',
    'default': '👍 Message received. I\'m here for you.'
  };

  var replyText = replies['default'];
  var lowerTrigger = triggerText.toLowerCase();
  if (lowerTrigger.indexOf('help') >= 0) replyText = replies['help'];
  else if (lowerTrigger.indexOf('safe') >= 0) replyText = replies['safe'];
  else if (lowerTrigger.indexOf('call') >= 0) replyText = replies['call'];
  else if (lowerTrigger.indexOf('unsafe') >= 0) replyText = replies['unsafe'];
  else if (lowerTrigger.indexOf('location') >= 0) replyText = replies['location'];
  ...
  setTimeout(function () {
    ...
    saveChatHistory(history);
    renderInAppChat();
    showToast('💬 ' + contact.name + ' replied!', 'info');
  }, delay);
}

// =============================================
// IN-APP AUDIO RECORDING (Chat)
// =============================================
function startChatAudioRecording() {
  var audioBar = document.getElementById('chat-audio-bar');

  navigator.mediaDevices.getUserMedia({ audio: true })
    .then(function (stream) {
      chatAudioStream = stream;
      chatAudioRecorder = new MediaRecorder(stream);
      chatAudioChunks = [];

      chatAudioRecorder.ondataavailable = function (e) {
        if (e.data && e.data.size > 0) chatAudioChunks.push(e.data);
      };

      chatAudioRecorder.onstop = function () {
        var blob = new Blob(chatAudioChunks, { type: 'audio/webm' });
        // Save the file
        downloadFile(blob, 'SHEild_Audio_' + Date.now() + '.webm');
        // Add as chat message
        addInAppMessage('Audio message recorded (' + formatTime(chatAudioSeconds) + ')', 'me', 'audio');
        showToast('🎤 Audio sent & saved!', 'success');
        simulateAutoReply('audio received');
      };

      chatAudioRecorder.start(1000);
      ...
      // Start timer
      chatAudioSeconds = 0;
      var timerEl = document.getElementById('chat-audio-timer');
      chatAudioTimerInterval = setInterval(function () {
        chatAudioSeconds++;
        if (timerEl) timerEl.textContent = formatTime(chatAudioSeconds);
      }, 1000);
...
function stopAndSendAudio() {
  if (chatAudioRecorder && chatAudioRecorder.state !== 'inactive') {
    chatAudioRecorder.stop();
  }
  if (chatAudioStream) {
    chatAudioStream.getTracks().forEach(function (t) { t.stop(); });
    chatAudioStream = null;
  }
  if (chatAudioTimerInterval) {
    clearInterval(chatAudioTimerInterval);
    chatAudioTimerInterval = null;
  }
...
function cancelAudioRecording() {
  if (chatAudioRecorder && chatAudioRecorder.state !== 'inactive') {
    chatAudioRecorder.ondataavailable = null;
    chatAudioRecorder.onstop = null;
    chatAudioRecorder.stop();
  }
  if (chatAudioStream) {
    chatAudioStream.getTracks().forEach(function (t) { t.stop(); });
...
function addChatMessage(text, channel) {
  var history = getChatHistory();
  var selectEl = document.getElementById('chat-contact-select');
  var contactTarget = selectEl ? selectEl.value : 'all';
  var contactName = 'All Contacts';
  if (contactTarget !== 'all') {
    var contacts = getContacts();
    var idx = parseInt(contactTarget, 10);
    if (contacts[idx]) contactName = contacts[idx].name;
  }
...
function renderChatMessages() {
  ...
  history.forEach(function (msg) {
    ...
    var channelIcon = msg.channel === 'whatsapp' ? '💬' : (msg.channel === 'sms' ? '📱' : '📤');
    bubble.innerHTML =
      '<div class="chat-bubble-text">' + escapeHtml(msg.text) + '</div>' +
      '<div class="chat-bubble-meta">' +
      '<span>' + channelIcon + ' → ' + escapeHtml(msg.contactName) + '</span>' +
      '<span>' + msg.time + '</span>' +
      '</div>';
    container.appendChild(bubble);
  });
...
function getSelectedContacts() {
  var selectEl = document.getElementById('chat-contact-select');
  var val = selectEl ? selectEl.value : 'all';
  var contacts = getContacts();

  if (val === 'all') return contacts;
  var idx = parseInt(val, 10);
  return contacts[idx] ? [contacts[idx]] : contacts;
}
...
function showSendOptions() {
  ...
  var bar = document.getElementById('send-channel-bar');
...
function sendChatViaWhatsApp() {
  var input = document.getElementById('chat-message-input');
  var text = input ? input.value.trim() : '';
  if (!text) return;

  var targets = getSelectedContacts();
  var encodedMsg = encodeURIComponent(text);
  ...
  if (targets.length === 0) {
    ...
    targets.forEach(function (c) {
      ...
      if (phone.charAt(0) !== '+') { phone = '91' + phone; } else { phone = phone.substring(1); }
      window.open('https://wa.me/' + phone + '?text=' + encodedMsg, '_blank');
    });
  }

  addChatMessage(text, 'whatsapp');
  input.value = '';
  hideSendOptions();
  showToast('💬 Sent via WhatsApp!', 'success');
}

function sendChatViaSMS() {
  var input = document.getElementById('chat-message-input');
  var text = input ? input.value.trim() : '';
  if (!text) return;
  ...
  if (targets.length === 0) {
    ...
    var phones = targets.map(function (c) { return c.phone; }).join(',');
    window.open('sms:' + phones + '?body=' + encodedMsg, '_self');
  }

  addChatMessage(text, 'sms');
  input.value = '';
  hideSendOptions();
  showToast('📱 Sent via SMS!', 'success');
}
...
function attachAndSendLocation() {
  showToast('📍 Getting location...', 'info');
  getRealLocation(function (lat, lng) {
    var mapsLink = 'https://www.google.com/maps?q=' + lat + ',' + lng;
    var locText = '📍 My current location:\nLat: ' + lat + ', Lng: ' + lng + '\n🗺️ ' + mapsLink;
    var input = document.getElementById('chat-message-input');
    if (input) {
      input.value = locText;
      showSendOptions();
...
function onChatContactChange() {
  // Could filter chat history by contact, for now just update UI
  var selectEl = document.getElementById('chat-contact-select');
  if (selectEl) {
    showToast('📨 Target: ' + selectEl.options[selectEl.selectedIndex].text, 'info');
  }
}

// =============================================
// CHAT PAGE — AUDIO RECORDING & SHARING
// =============================================
function recordAndShareAudio() {
  var modal = document.getElementById('audio-share-modal');
  ...
  navigator.mediaDevices.getUserMedia({ audio: true })
    .then(function (stream) {
      ...
      chatAudioRecorder.onstop = function () {
        ...
        // Show share controls
        var recordControls = document.getElementById('audio-record-controls');
        var shareControls = document.getElementById('audio-share-controls');
        var titleEl = document.getElementById('audio-modal-title');
        var statusEl = document.getElementById('audio-modal-status');
        if (recordControls) recordControls.style.display = 'none';
        if (shareControls) shareControls.style.display = 'block';
        if (titleEl) titleEl.textContent = 'Audio Ready!';
        if (statusEl) statusEl.textContent = 'Share your audio evidence with contacts.';
      };
      ...
      if (modal) {
        ...
        var titleEl = document.getElementById('audio-modal-title');
        var statusEl = document.getElementById('audio-modal-status');
        if (recordControls) recordControls.style.display = 'block';
        if (shareControls) shareControls.style.display = 'none';
        if (titleEl) titleEl.textContent = 'Recording Audio...';
        if (statusEl) statusEl.textContent = 'Recording audio evidence...';
      }

      // Start timer
      chatAudioSeconds = 0;
      var timerEl = document.getElementById('audio-share-timer');
      chatAudioTimerInterval = setInterval(function () {
        chatAudioSeconds++;
        if (timerEl) timerEl.textContent = formatTime(chatAudioSeconds);
      }, 1000);
...
function shareAudioViaWhatsApp() {
  // Save audio file first
  if (chatAudioBlob) {
    downloadFile(chatAudioBlob, 'SHEild_Audio_' + Date.now() + '.webm');
  }

  // Send text message via WhatsApp indicating audio was recorded
  var lat = currentLat || '—';
  var lng = currentLng || '—';
  var msg = '🎤 SHEild AI — Audio Evidence\n\n' +
    'I have recorded an audio evidence file.\n' +
    '📍 Location: https://www.google.com/maps?q=' + lat + ',' + lng + '\n' +
    '⏰ Time: ' + new Date().toLocaleString() + '\n' +
    'Duration: ' + formatTime(chatAudioSeconds) + '\n' +
  ...
  addChatMessage('🎤 Audio evidence recorded (' + formatTime(chatAudioSeconds) + ') — shared via WhatsApp', 'whatsapp');
  closeModal('audio-share-modal');
  showToast('🎤 Audio saved & shared via WhatsApp!', 'success');
}

function shareAudioViaSMS() {
  if (chatAudioBlob) {
    downloadFile(chatAudioBlob, 'SHEild_Audio_' + Date.now() + '.webm');
  }
  ...
  addChatMessage('🎤 Audio evidence recorded (' + formatTime(chatAudioSeconds) + ') — shared via SMS', 'sms');
  closeModal('audio-share-modal');
  showToast('🎤 Audio saved & shared via SMS!', 'success');
}

function downloadAudioEvidence() {
  if (chatAudioBlob) {
    downloadFile(chatAudioBlob, 'SHEild_Audio_' + Date.now() + '.webm');
    showToast('📥 Audio evidence saved!', 'success');
  }
